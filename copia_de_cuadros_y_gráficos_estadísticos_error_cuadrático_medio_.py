...
Original file is located at
    https://colab.research.google.com/drive/18VOAtRTzGX-bSwNMyPBNElLY1eKp7GN-
...
# Datos de porcentajes de juegos ganados para las seis divisiones
data = [
...
df = pd.DataFrame({
    'Porcentaje de Juegos Ganados': data
})

clase = pd.qcut(df['Porcentaje de Juegos Ganados'], q=5, labels=['Muy bajo', 'Bajo', 'Medio', 'Alto', 'Muy alto'])

df['Clasificacion'] = clase

print("Datos clasificados:")
print(df)

frecuencia_absoluta = df['Clasificacion'].value_counts()
frecuencia_relativa = df['Clasificacion'].value_counts(normalize=True)
...
plt.plot(frecuencia_absoluta.index, frecuencia_absoluta.values, marker='o', linestyle='-', color='blue')
...
frecuencia_acumulada = frecuencia_absoluta[::-1].cumsum()[::-1]


plt.figure(figsize=(8,5))
plt.plot(frecuencia_acumulada.index, frecuencia_acumulada.values, marker='o', linestyle='-', color='red', label='Frecuencia Acumulada Mayor Que')
plt.xlabel('Clases')
plt.ylabel('Frecuencia Acumulada')
